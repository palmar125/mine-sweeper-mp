...
void showEndResult(WINDOW *winResult, WINDOW *winStatus, fieldStruct *field, pthread_t clockPtr, char *str)
{
    mvwprintw(winResult, 0, 1, "%s",str);
    wrefresh(winResult);
    free(field);
    clockStop = 1;
    ...
    while ((ch = getch()))
    {
        ...
        case 'F':
            ...
            if (field[fieldy * gameW + fieldx].flag == 0 && field[fieldy * gameW + fieldx].opened != 1)
            {
                ...
                {
                    curs_set(0);
                    showEndResult(winResult,winStatus,field, clockPtr, "YOU WON");                    
                    return (1);
                }                
            }
            else
        ...
        case 10: //enter key
            ...
            else
            {
                ...
                { /* -1 is a mine*/
                    ...
                    wmove(stdscr, starty + fieldy, startx + fieldx * 2);
                    // You pressed a mine
                    showEndResult(winResult,winStatus,field, clockPtr, "YOU LOST");                    
